...
VkDescriptorSet DescriptorAllocatorGrowable::allocate(
  ...
  VkDescriptorSetAllocateInfo alinf{};
  alinf.pNext = pnext;
  alinf.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
  alinf.descriptorPool = poolToUse;
  alinf.descriptorSetCount = 1;
  alinf.pSetLayouts = &layout;

  VkDescriptorSet ds;
  VkResult res = vkAllocateDescriptorSets(device, &alinf, &ds);

  if (res == VK_ERROR_OUT_OF_POOL_MEMORY || res == VK_ERROR_FRAGMENTED_POOL) {
    fullPools.push_back(poolToUse);
    poolToUse = get_pool(device);
    alinf.descriptorPool = poolToUse;
    VK_CHECK(vkAllocateDescriptorSets(device, &alinf, &ds));
  }
